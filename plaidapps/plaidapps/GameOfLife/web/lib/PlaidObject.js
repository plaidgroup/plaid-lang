...
PlaidObject.prototype.clone = function() {
		var obj = (this instanceof Array) ? [] : new PlaidObject();
  var i;
  for (i in this) {
    if (i == 'clone') continue;
    if (this[i] && typeof this[i] == "object") {
...
function m_stateChange(obj1,obj2){
   ...
   for (var j=0;j<removeLength;j++){
      //document.write("remove "+remove[j]+"<br>");
			 if(remove[j].indexOf("off") == 0){
					 obj1[remove[j]]();
			 }
			 if(!(remove[j].indexOf("off") == 0 || remove[j].indexOf("on") == 0)){
...
PlaidObject.prototype.freeze=function() {
   ...
   for (i in this) {
      if ( i==="tree" || i==="match" || i==="tags" || i==="members" || i=="membersUnique" || i=="stateChange" || i==="stateChangeMember" || i==="stateChangeMemberNoValue" || i==="replace" || i==="freeze" || i==="clone" || i==="begetObj") {
         continue;
      }
      else {
