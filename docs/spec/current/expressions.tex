...
right, in which case those on the right override them.  \pII{If this
  results in a member being incompatible with an abstract member
  of a remaining tag, an \code{IllegalStateChangeException} is
  thrown.}
...
\defspace \keyw{branded}

\vII{\defspace \keyw{immutable}}
...
\keyw{branded} indicates that the type being defined is nominal, and
...
\vII{\keyw{immutable} means a state is immutable and that any variables
in that state have permission immutable by default}
