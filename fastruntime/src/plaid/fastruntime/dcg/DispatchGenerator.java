...
public final class DispatchGenerator implements Opcodes {
	...
	private final Map<String, PlaidDispatch> dispatchCache = new WeakHashMap<String,PlaidDispatch>();
	private final Map<String, String> internStringMap = new HashMap<String, String>();

	public PlaidDispatch createStateInstance(ObjectValue ov) {
		String canonicalRep = ov.getCanonicalRep();
		if (internStringMap.containsKey(canonicalRep)) {
			canonicalRep = internStringMap.get(canonicalRep);
			if(dispatchCache.containsKey(canonicalRep)) {
				return dispatchCache.get(canonicalRep);
			}
		} else {
			String interned = canonicalRep.intern();
			internStringMap.put(interned, interned);
		}
		...
		dispatchCache.put(ov.getCanonicalRep(), result);
		return result;
